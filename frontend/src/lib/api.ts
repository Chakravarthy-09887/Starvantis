...
async function fetchApi<T>(endpoint: string, options?: RequestInit, fallback?: T | (() => T)): Promise<T> {
  ...
  const executeFetch = async (attempt = 1): Promise<T> => {
    ...
    try {
      ...
      clearTimeout(timeoutId);
      // Retry once on connection failure or cold start
      if (attempt < 2 && (err.name === 'AbortError' || err.message?.includes('network') || err.message?.includes('Failed to fetch') || err.message?.includes('Load failed'))) {
...
export function getChandrayaan3DeepSpaceFallback(): Chandrayaan3DeepSpaceData {
  ...
  return {
    ...
    chaste_surface_temp_c: parseFloat((50.4 + Math.sin(t * 0.05) * 4.2).toFixed(1)),
    ...
    rambha_plasma_density_cm3: 10500,
    rover_pragyan_distance_traversed_m: 101.4,
    battery_charge_pct: 98.5,
  };
}

export function getAdityaL1DeepSpaceFallback(): AdityaL1DeepSpaceData {
  const t = Date.now() / 3600000;
  return {
    ...
    solar_uv_suit_flux_wm2: parseFloat((1361.2 + Math.sin(t * 0.2) * 1.8).toFixed(2)),
    aspex_proton_alpha_ratio_pct: parseFloat((4.25 + Math.cos(t * 0.3) * 0.4).toFixed(2)),
    plasma_speed_kms: parseFloat((482.4 + Math.sin(t * 0.1) * 24.0).toFixed(1)),
    triaxial_mag_field_nt: {
      Bx: parseFloat((3.42 + Math.sin(t * 0.4) * 0.8).toFixed(2)),
      By: parseFloat((-1.85 + Math.cos(t * 0.3) * 0.6).toFixed(2)),
      Bz: parseFloat((-3.80 + Math.sin(t * 0.2) * 1.2).toFixed(2)),
    },
    halo_orbit_phase_deg: parseFloat(((t * 2.5) % 360.0).toFixed(1)),
    station_keeping_delta_v_ms_yr: 2.45,
  };
}

export function getJWSTDeepSpaceFallback(): JWSTDeepSpaceData {
  const t = Date.now() / 3600000;
  return {
    spacecraft_id: 'JWST',
    ...
    sunshield_hot_side_temp_c: parseFloat((85.2 + Math.sin(t * 0.1) * 2.1).toFixed(1)),
...
export function getSpaceWeatherFallback(): SpaceWeatherIndices {
  const t = Date.now() / 3600000;
  return {
    timestamp: new Date().toISOString(),
    kp_index: parseFloat((2.33 + Math.sin(t * 0.1) * 0.5).toFixed(2)),
    storm_level: 'G0 - QUIET',
    storm_category: 'Nominal Magnetosphere',
    solar_wind_speed_kms: parseFloat((412.5 + Math.sin(t * 0.08) * 18.0).toFixed(1)),
    solar_wind_density_pcm3: parseFloat((4.8 + Math.cos(t * 0.15) * 0.6).toFixed(1)),
    solar_wind_pressure_npa: 1.68,
    magnetopause_standoff_re: 10.4,
    imf_bz_nt: parseFloat((1.2 + Math.sin(t * 0.25) * 1.5).toFixed(2)),
    imf_bt_nt: 4.8,
    dst_index_nt: -12.4,
    auroral_power_gw: 14.2,
    goes_xray_flux: 'B4.2 (Sub-flare Baseline)',
    flare_class: 'B-Class Low',
    radio_flux_f107: 142.8,
    aditya_l1_stream: 'CONNECTED // L1 Halo Orbit Active',
    saa_status: 'NOMINAL_MONITORED',
    van_allen_inner_flux: '2.4 × 10³ e⁻/cm²·s',
    van_allen_outer_flux: '1.8 × 10⁴ e⁻/cm²·s',
  };
}

export function getSpacecraftRadiationFallback(satelliteId: string): SpacecraftRadiationDose {
  return {
    ...
    van_allen_region: 'OUTSIDE_BELTS',
    ambient_flux_pcm2s: 340.0,
    cumulative_dose_krad: 14.8,
    tid_limit_krad: 100.0,
    tid_health_pct: 85.2,
    seu_risk_level: 'LOW',
    edac_scrub_rate_hz: 10.0,
    solar_cell_degradation_pct: 2.8,
    recommended_mitigation: 'Nominal scrub cycle active; no SAA shield command required.',
  };
}
...
export const api = {
  ...
  getSpaceWeather: () => fetchApi<SpaceWeatherIndices>('/space-weather/live', undefined, getSpaceWeatherFallback),
  getSpacecraftRadiation: (satelliteId: string) =>
    fetchApi<SpacecraftRadiationDose>(`/space-weather/radiation/${satelliteId}`, undefined, () => getSpacecraftRadiationFallback(satelliteId)),

  // Deep-Space & Lagrange Point Specialized Displays
  getAdityaL1DeepSpace: () => fetchApi<AdityaL1DeepSpaceData>('/deep-space/aditya-l1', undefined, getAdityaL1DeepSpaceFallback),
  getChandrayaan3DeepSpace: () => fetchApi<Chandrayaan3DeepSpaceData>('/deep-space/chandrayaan-3', undefined, getChandrayaan3DeepSpaceFallback),
  getJWSTDeepSpace: () => fetchApi<JWSTDeepSpaceData>('/deep-space/jwst', undefined, getJWSTDeepSpaceFallback),
